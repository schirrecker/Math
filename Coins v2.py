...
class History(list):
    ...
    def tryMove(self, move, tried=False):
        self.trials += 1
        if self.current_row.solved():
            ...
            if tried == True:
                # print ("Already tried " + str(move))
                if lastPossibleMove(move):
                    self.backtracks += 1
                    ...
                    print("")
                    self.pprint()
                    # print ("Last move was: " + str(last_move))
                    if lastPossibleMove(last_move): 
                        self.tryMove(nextPossibleMove(last_move), tried=True)
                    else:
                        self.tryMove(nextPossibleMove(last_move))     
             
            else:
                # print("Trying " + str(move) + "   number:" + str(possibleMoves.index(move)+1))
                if self.current_row.moving(move) and tried == False:
                    new_row = Row(self.size)
                    new_row.copyRow(self.current_row)
                    self.append(self.Record(new_row, move))
                    self.pprint()
                    self.tryMove(possibleMoves[0], tried=False)
                    ...
                    if lastPossibleMove(last_move):
                        ...
                        self.tryMove(nextPossibleMove(last_move), tried=False)     
                else:
                    self.tryMove(nextPossibleMove(move), tried=False)
